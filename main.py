...
class Config:
    # 数据参数
    feature_columns = [0, 1, 2, 3, 4, 7, 8, 9, 10, 11]#list(range(2, 9))     # 要作为feature的列，按原数据从0开始计算，也可以用list 如 [2,4,6,8] 设置
    ...
    hidden_size = 30           # LSTM的隐藏层大小，也是输出大小
...
class Data:
    ...
    def read_data(self):                # 读取初始数据
        if self.config.debug_mode:
            ...
            init_data.values[int(init_data.shape[0] * self.config.train_data_rate):, 5:] = init_data.values[int(init_data.shape[0] * self.config.train_data_rate):, 2:5]* expected
    ...
    def get_test_data(self, return_label_data=False):
        ...
        # test_x = [feature_data[i:i + self.config.time_step] for i in range(self.train_num + self.start_num_in_test - self.config.time_step)]
        # print(np.shape(test_x))   (3,30,8)
...
def draw(config: Config, origin_data: Data, logger, predict_norm_data: np.ndarray):
    ...
    # 保存到excel
    data1 = pd.DataFrame(predict_data)
    writer = pd.ExcelWriter('./data/result.xlsx')
    data1.to_excel(writer, 'sheet1', float_format='%.5f')
    writer.save()
    writer.close()
